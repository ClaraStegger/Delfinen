...
public class Main {
    ...
    private static void searchForMember(List<Member> members) {
        Scanner scan = new Scanner(System.in);
        System.out.println("Enter the phone number of the Member: ");
        String phoneNumberOfMemberToSearchFor = scan.nextLine();
        boolean found = false;
        for (int i = 0; i < members.size(); i++) {
            Member member = members.get(i);
            if (member.getPhoneNumber().equals(phoneNumberOfMemberToSearchFor)) {
                System.out.println("\n" + member + "\n");
                found = true;
                break;
            }
        ...
        if (!found) {
            System.out.println("\nA Member with the entered phone number, does not exit. Try again!\n");
        }
    }
...
private static void updateMoneyOwed(Member member, LocalDate startDate) {
    ...
    System.out.printf("Has the member paid their starting fee of %.2f kr. Enter yes/no\n", moneyOwed);
